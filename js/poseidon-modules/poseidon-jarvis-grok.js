...
(function () {
  ...
  const TOOLS = [
    ...
    },
    {
      ...
      parameters: {
        ...
        properties: {
          ...
          source:    { type: 'string',  enum: ['all', 'craigslist', 'airbnb', 'vrbo', 'owner', 'zillow', 'apartments', 'facebook', 'furnishedfinder'], description: 'Source tab to activate. "owner" = Rent by Owner. "apartments" = Apartments.com. "facebook" = Facebook Marketplace. "furnishedfinder" = Furnished Finder.' },
    ...
    },
    {
      type: 'function',
      name: 'query_skill',
      description: 'Search a specific Jarvis Skills agent domain for knowledge. With a topic, returns matching KB entries (full topic, content, status, partition notes, source docs). WITHOUT a topic, returns ALL knowledge-base entries with their FULL content — use this to summarize or explain an entire domain in depth. STRICT PARTITION: the recruiting-j1 domain is REFERENCE ONLY — operational J-1 data lives on j1-system-dashboard.html (use read_remote_dashboard for that). The cruise-staffing domain is cruise/maritime only.',
      parameters: {
        type: 'object',
        properties: {
          domain: {
          ...
          topic: { type: 'string', description: 'Optional. Free-text topic to match against KB entry topics, ids, and content. Omit to retrieve ALL entries for in-depth domain explanation.' }
        },
        required: ['domain']
      }
    },
    {
      ...
      description: 'Read the COMPLETE Jarvis Skills agent domain in one call: domain metadata (owner, version, refresh frequency, partition warning), every knowledge-base entry with full content, every decision tree with its rules and escalation path, every data source with status, every domain-level escalation rule, AND the global escalation rules. Use this whenever Robert asks Jarvis to "explain", "tell me about", "summarize", "go deep on", or "walk me through" any of the six agent domains. This is the canonical way to answer in-depth Skills Library questions — DO NOT use read_page_content for Skills questions; use this instead.',
      parameters: {
        type: 'object',
        properties: {
          domain: {
            type: 'string',
            enum: ['tax-compliance','recruiting-j1','cruise-staffing','marketing-bd','operations-process','ghr-platform'],
            description: 'Agent domain id to read in full.'
          }
        },
        required: ['domain']
      }
  ...
  const TOOL_IMPL = {
      ...
      try {
        ...
        return {
          ...
          available_options: {
            ...
            sources: ['all', 'craigslist', 'airbnb', 'vrbo', 'owner', 'zillow', 'apartments', 'facebook', 'furnishedfinder'],
    ...
    async read_skill_domain({ domain }) {
      const ns = window.JarvisSkills;
      if (!ns) return { ok: false, error: 'JarvisSkills namespace not loaded yet.' };
      if (!ns.getCache().index) { try { await ns.loadAll(); } catch (e) { return { ok: false, error: 'loadAll failed: ' + (e && e.message) }; } }
      return ns.readDomain(domain);
    },
