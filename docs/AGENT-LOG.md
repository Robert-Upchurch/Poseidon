...
2026-05-16  Implementor  feat/button-conversion  Canonical button conversion — 3 dashboards converted (2 cti pilots + master), 6 bespoke files flagged unchanged, merged to main via --no-ff.
  ...
  · Flagged UNCHANGED (bespoke Tailwind/custom styling — canonical reclass = regression risk; conservative mandate): poseidon-dashboard-v6 (121 btns; theme = rounded-full switch), j1-system-dashboard (121), poseidon-dashboard (defines own .btn base — hard collision, legacy), j1-housing-finder-index, j1-housing-dashboard, tracker. poseidon-dashboard-v5 skipped (frozen rollback). index.html 0 buttons. Each needs its own scoped plan before any canonical adoption.
  · Evidence: 26 per-tab before/after PNGs via Playwright in gitignored button-conversion/ sandbox (guard.js route-aborted for shell render; hash-driven tab nav). Screenshots CAPTURED for the 3 checks (toolbar icons compact, table row-delete trash not ballooned, Save/Add canonical teal) — PENDING user review, not yet user-confirmed.
  · Merge: git merge --no-ff feat/button-conversion → main (Phase-3 flow). Purely additive class swaps + local-def removal + shared .icon-btn; no bespoke/flagged file modified.

2026-05-15 afternoon  Implementor  main  Phase 3 M365 SSO COMPLETE — first live sign-in verified, feat/m365-sso merged to main, production on main.
  · Verification: Robert signed in as ceo@cti-usa.com in Incognito; j1-housing-finder-index.html rendered with data (not bounced to login). guard.js redirects to login on ANY auth failure, so a rendered protected page = full JWT round-trip through checkSSO() returned 200 = `auth: sso-ok` server-side. Phase 1's previously-untested Layer-A JWT path is now proven end-to-end with a real Entra id_token.
  · First sign-in failure root cause: auth/config.js MSAL_CDN pointed at legacy alcdn.msauth.net (MSAL v1 host) which 404s for v3.6.0. Fixed in commit bb989c1 → jsdelivr (cdn.jsdelivr.net/npm/@azure/msal-browser@${MSAL_VERSION}/lib/msal-browser.min.js), matching every other Poseidon dashboard.
  · Server-side verification matrix (no browser, curl only): guard.js <script> present on live j1 page; live auth/config.js serves jsdelivr URL @3.6.0; MSAL bundle returns 200 (was the 404); Worker /api/kpi-summary unauth → 401 with checkSSO hint (gate enforcing); POSEIDON_EMAIL_ALLOWLIST=ceo@cti-usa.com in wrangler.toml; allowlist.js fail-closed (empty var = deny-all, never allow-all).
  · Merge: git merge --no-ff feat/m365-sso → main, merge commit d9c5dba. 9 commits, 17 files, +1306/−0 — purely additive (no existing main code modified; only new auth/ + poseidon-worker/ + the 1-line guard tag on the j1 page). GH Pages source flipped feat/m365-sso → main as part of this close; production re-verified post-flip.
  · Operational constraint (cost a full detour this session): wrangler tail AND wrangler login do NOT work from Claude Code's non-interactive shell — login's browser OAuth times out when launched as a background task; tail fails 401 (stored OAuth token expired, and there is no CLOUDFLARE_API_TOKEN env var or poseidon-worker/.dev.vars on this machine). Future SSO verification = server-side curl + one real Robert sign-in, NOT live Worker log tailing. Saved to memory.
  · Phase 4 (next resume point, ~1–2 hr): retrofit remaining dashboards with SSO — add `<script type="module" src="/Poseidon/auth/guard.js"></script>` to each (poseidon-dashboard-v6.html + the rest). Phase 2 design makes this a one-line tag per dashboard; no Worker change needed (checkSSO already gates all /api/*).
