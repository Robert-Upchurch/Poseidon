/* CTI Cruise Portal - shared client JS
...
(function () {
  ...
  var THEME_KEY = 'cti-cruise-theme';
  ...
  // Pre-paint (called inline in <head> via window.__ctiPreTheme too)
  ...
  function wireStepper() {
    ...
    function showStep(idx) {
      for (var i = 0; i < panes.length; i++) {
        panes[i].classList.toggle('active', i === idx);
      }
      for (var j = 0; j < steps.length; j++) {
        var num = steps[j].querySelector('.step-num');
        if (j <= idx) {
          steps[j].classList.add('step-active');
          steps[j].classList.remove('step-idle');
          if (num) num.style.background = '#ffffff', num.style.color = '#14b8a6';
        } else {
          steps[j].classList.add('step-idle');
          steps[j].classList.remove('step-active');
          if (num) num.style.background = '', num.style.color = '';
        }
      }
      // Build review pane content when entering last step
  ...
  function wireTrack() {
    ...
    var portal = root.getAttribute('data-portal') || 'cruise';
  ...
  function renderTrackStatus(slot, data) {
    ...
    if (housing) {
      ...
      html += '<p style="margin:8px 0 0;">' + escapeHtml(housing.summary || '') + '</p>';
      html += '</div>';
    }
    html += '</div>';
    slot.innerHTML = html;
